...
**ColBERT-based embedding** - using SentenceSplitter - **and Retrieval** proved to be the **most efficient method of improving result accuracy and relevance**, with an average performance advantage of about 10% over the second best method. This was true on all the datasets we tested.

This result surprised us. We intuitively assumed that the naive **SentenceSplitter** (which takes in chunk_size and overlap parameters) would be the _least_ efficient chunking method, and that instead the **SemanticSplitterNodeParser**’s performance would be _superior_ - because, for any given sentence, it creates chunks based on breakpoints computed from semantic dissimilarities of the preceding and succeeding sentences. This assumption turned out to be false. **Why?**
